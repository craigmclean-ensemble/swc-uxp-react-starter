...
const OAUTH_HASH_ALGORITHM = 'SHA-256';
...
export class OAuthUtils {
  ...
  async getAccessToken(id) {
    ...
    const raw = await this._store.getItem(key);
    const tokenResponseString = storageValueToString(raw);
...
async function saveTokenResponse(store, id, clientId, formData, tokenUrl) {
  ...
  const key = typeof id === 'string' ? id : String(id);
  await store.setItem(key, JSON.stringify(tokenResponse));
